function [y,X,X_spline,theta,w,Xw,nustar ] = synthesizeDataset( param,synth)
...
for tt=1:T
    ...
    for j = 1 : P
      ...
      X_trn_spline(:,(j-1)*r+1:j*r) = bsplinebasis(X_trn(:,j), knot_trn, synth.features.order);
...
x = x.*nu';
index = w~=0;
y_star =sum(2.5*sin(x(:,1:10))*index(1:10)+2*x(:,11:20)*index(11:20)+(2*exp(x(:,21:30))+exp(-1)-1)*index(21:30)+8*(x(:,31:40)).^2*index(31:40)+3*sin(2*exp(x(:,41:50)))*index(41:50),2);
